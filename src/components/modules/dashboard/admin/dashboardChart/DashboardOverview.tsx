...
export default function DashboardOverview() {
 ...
 const { data: usersData, isLoading: usersLoading, refetch } = useQuery({
  ...
  const stats = useMemo(() => {
    ...
    const hasData = eventList.length > 0 || totalUsers > 0;
    ...
    return {
      ...
      pieData: hasData
        ? [
            ...
            { name: "Users", value: totalUsers, color: "#0ea5e9" }, 
          ]
        : [{ name: "No Data", value: 1, color: "#f1f5f9" }],

      chartData: [
        { name: "Events", count: eventList.length },
        { name: "Users", count: totalUsers },
        { name: "Reviews", count: reviewList.length || 0 },
      ],
    };
  }, [events, usersData, notifications, reviewsData]);

  if (!isMounted || eventsLoading || notifLoading || usersLoading) {
    return (
    ...
    <div className="p-6 space-y-6 bg-slate-50/40 min-h-screen font-sans">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-slate-500 font-medium">
          Monitoring your event ecosystem in real-time.
        </p>
      </div>
              ...
              {stats.pieData.map((item) => (
                <div key={item.name} className="flex flex-col items-center">
                  ...
                  <span className="text-sm font-black text-slate-800">
                    {item.name === "No Data" ? 0 : item.value}
                  </span>
                </div>
              ))}
            </div>
        ...
        <CardHeader className="flex flex-row items-center justify-between border-b border-slate-50">
          ...
          <Link href="admin/events">
            <Button
              variant="outline"
              size="sm"
              className="rounded-full font-bold text-xs border-slate-200"
            >
              See All
            </Button>
          </Link>
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow>
                <TableHead className="font-bold uppercase text-[10px] pl-6">
                  Title
                </TableHead>
                <TableHead className="font-bold uppercase text-[10px]">
                  Date
                </TableHead>
                <TableHead className="text-right font-bold uppercase text-[10px] pr-6">
                  Action
                </TableHead>
              </TableRow>
            </TableHeader>
              ...
              {stats.recentEvents.length > 0 ? (
                stats.recentEvents.map((event: any) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Badge className="font-bold text-[9px] uppercase px-2 py-0.5 rounded-md bg-indigo-50 text-indigo-700 shadow-none border-none">
                    ...
                    <TableCell className="text-right pr-6">
                      <Link href={`/events/${event.id}`}>
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-8 w-8 rounded-full group-hover:bg-indigo-600 group-hover:text-white"
                        >
