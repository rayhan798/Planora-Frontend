...
import { getInvitations, getMyProfile, logoutAction } from "@/app/(public)/events/_actions"; 
import { 
  DropdownMenu, 
  DropdownMenuContent, 
  DropdownMenuItem, 
...
export default function Header({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  // ✅ ভ্যালিড ইউজার চেক (userData.name আছে কি না তা দেখা)
  ...
  const { data: invitesData } = useQuery({
    ...
    enabled: !!user, // শুধুমাত্র ইউজার থাকলে ইনভিটেশন ফেচ করবে
  });
    ...
    <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-6 sticky top-0 z-30">
      ...
      <div className="flex items-center gap-3">
        ...
        <Link href="/dashboard/pending-invitations">
          <Button variant="outline" size="icon" className="rounded-xl relative hover:bg-slate-50 border-slate-200">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56 p-2 rounded-2xl border-slate-100 shadow-2xl animate-in slide-in-from-top-2">
            <div className="px-3 py-2">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Signed in as</p>
              <p className="text-xs font-bold text-slate-700 truncate">{user?.email || "User"}</p>
            </div>
